...
const fs = require('fs')
const mainGeneratorPathLocation = path.join(__dirname, '/generators/app')
...
describe('mkroosevelt:mainGenerator', function () {
  ...
  afterEach(function () {
    let packageJson = JSON.parse(fs.readFileSync(tempDir + '/package.json', 'utf8'))
    // runs after each test in this block
    let validatePackageJson = JsonValidator({
      required: true,
      type: 'object',
      properties: {
        name: {
